...
class TrackExample():
    ...
    def before_fit(self):
        ...
        self.val_example_x_text=clean_text(self.tokenizer.decode(val_example_x, skip_special_tokens=False))
        self.val_example_y_text=clean_text(self.tokenizer.decode(val_example_y, skip_special_tokens=False))

    def after_epoch(self):
        if not self.learner.model.training:
            train_example_y_pred_text=clean_text(self.learner.model.translate(self.train_example_x_text, self.tokenizer))
            val_example_y_pred_text=clean_text(self.learner.model.translate(self.val_example_x_text, self.tokenizer))

            _string = f"""Tracking Example progress:
            ...
            """
            log.info(_string.encode().decode('gbk'))
...
class TrackBleu():
    ...
    def after_batch(self):
        if not self.learner.model.training:
            ...
            preds=self.learner.preds.detach().cpu().float()
    ...
    def after_epoch(self):
        if not self.learner.model.training:
            yb_text_tokens_for_bleu=[[clean_text(item)] for item in self.yb_text_tokens]
            self.preds_text_tokens = [clean_text(text) for text in self.preds_text_tokens]
            bleu=corpus_bleu(yb_text_tokens_for_bleu,self.preds_text_tokens)
            _log = {'bleu': bleu, "BLEU": math.exp(bleu*4)}
            wandb.log(_log, step=self.learner.cur_step)
            log.info(_log)
...
#         if self.learner.model.training and self.learner.cur_step % 10 == 0:
        wandb.log({
            'batch':self.learner.batch_idx, 
            'epoch':self.learner.epoch_idx,
            'Loss/Train': loss.item()}, step=self.learner.cur_step)
    ...
    def after_epoch(self):
        ...
        if self.learner.model.training:
            ...
            t1 = time.time() - self.epoch_time
            self.epoch_time = time.time()
            wandb.log({"Time/Total_Time(seconds)": t0, "Time/Epoch_Time(seconds)": t1, 'Loss/Train_Avg': avg_loss}, step=self.learner.cur_step)
        else:
            log.info(f"Epoch: {self.learner.epoch_idx} | Validation | Loss: {avg_loss:.5f}")
            wandb.log({'Loss/Val_avg': avg_loss}, step=self.learner.cur_step)
