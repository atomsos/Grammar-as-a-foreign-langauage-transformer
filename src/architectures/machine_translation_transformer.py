...
class MachineTranslationTransformer(nn.Module):
    ...
    def forward(self, src_token_ids, trg_token_ids):
        ...
        # Since trg_token_ids contains both [BOS] and [EOS] tokens
        # we need to remove the [EOS] token when using it as input to the decoder.
        # Similarly we remove the [EOS] token when we use it as y to calculate loss,
    ...
    def translate(self, sentence, tokenizer, max_tokens=301, skip_special_tokens=False):
